-- Domani only. Restricted staff projections; no telemetry backfill.
BEGIN;
REVOKE ALL ON public.profiles_dashboard FROM PUBLIC, anon, authenticated;
GRANT SELECT ON public.profiles_dashboard TO service_role;
-- Invoker views need only these auth columns, never tokens or password fields.
GRANT SELECT(id,created_at,last_sign_in_at,email_confirmed_at,deleted_at,banned_until) ON auth.users TO service_role;
GRANT SELECT(user_id,provider) ON auth.identities TO service_role;
CREATE INDEX IF NOT EXISTS domani_feedback_user_observation ON public.beta_feedback(user_id,created_at DESC,id);
CREATE INDEX IF NOT EXISTS domani_support_user_observation ON public.support_requests(user_id,created_at DESC,id);
CREATE VIEW public.dashboard_domani_user_devices WITH (security_invoker=true) AS
 SELECT user_id,id AS source_id,'feedback'::text AS source,created_at AS observed_at,platform,device_brand,device_model,os_version,app_version,app_build FROM public.beta_feedback
 WHERE device_model IS NOT NULL OR platform IS NOT NULL OR app_version IS NOT NULL
 UNION ALL
 SELECT user_id,id,'support',created_at,platform,device_brand,device_model,os_version,app_version,app_build FROM public.support_requests
 WHERE device_model IS NOT NULL OR platform IS NOT NULL OR app_version IS NOT NULL;
CREATE VIEW public.dashboard_domani_user_insights WITH (security_invoker=true) AS
 ...
 a.created_at AS joined_at,a.last_sign_in_at,p.last_active_at,
 CASE WHEN p.last_active_at IS NOT NULL THEN 'app_foreground' END AS activity_source,
 ...
 CASE WHEN d.source_id IS NOT NULL THEN to_jsonb(d)-'user_id' END AS latest_device_observation,
...
REVOKE ALL ON public.dashboard_domani_user_devices,public.dashboard_domani_user_insights FROM PUBLIC,anon,authenticated;
GRANT SELECT ON public.dashboard_domani_user_devices,public.dashboard_domani_user_insights TO service_role;

CREATE FUNCTION public.list_dashboard_domani_users(p_query jsonb DEFAULT '{}') RETURNS jsonb
...
BEGIN
 ...
 WHEN 'recent' THEN u.last_active_at>=now()-interval '30 days' AND u.last_active_at<=now()
 ...
 count(*) FILTER(WHERE account_status='active' AND last_active_at>=now()-interval '30 days' AND last_active_at<=now()) AS active_30d FROM filtered)
 SELECT jsonb_build_object('items',coalesce((SELECT jsonb_agg(to_jsonb(page)-'reported_platform'-'reported_app_version' ORDER BY %I %s NULLS LAST,id ASC) FROM page),'[]'::jsonb),
