...
"$pg_bin/psql" -h "$fixture_dir" -U postgres -d postgres -v ON_ERROR_STOP=1 \
  ...
  -f "$repo/supabase/migrations/20260919142130_domani_feedback_inbound.sql" \
  -f "$repo/supabase/tests/domani_feedback_inbound_assertions.sql"
echo "Inbound SQL tests passed"
