...
interface MutationRpcResult {
    ...
    cacheInvalidation: { jobId: string; status: 'pending' | 'delivered'; targets: string[] } | null
}

const actor = (req: Request) => {
    if (!req.dashboardActor) throw new AdminReleaseApiError(401, 'AUTH_REQUIRED', 'Authentication is required')
    return req.dashboardActor
}

const sendError = (req: Request, res: Response, error: unknown, label: string): Response => {
    if (error instanceof AdminReleaseApiError) return adminReleaseErrorResponse(req, res, error.status, error.code, error.message, error.fieldErrors)
    console.error(`Admin release ${label} failed:`, { requestId: req.requestId, message: error instanceof Error ? error.message : String(error) })
    return adminReleaseErrorResponse(req, res, 500, 'INTERNAL_ERROR', `Unable to ${label}`)
}

const successMeta = (requestId: string, next: string | null = null, cacheInvalidation?: MutationRpcResult['cacheInvalidation']) => ({
...
const sendMutation = (res: Response, requestId: string, result: MutationRpcResult, status = 200): Response => {
    res.setHeader('Cache-Control', 'no-store')
    res.setHeader('ETag', `"${result.primaryRowVersion}"`)
    res.setHeader('X-Release-ETag', `"${result.releaseRowVersion}"`)
    return res.status(status).json({ data: result.data, meta: successMeta(requestId, null, result.cacheInvalidation) })
}

const querySubset = (query: Request['query'], keys: string[]): Record<string, unknown> =>
    Object.fromEntries(keys.filter(key => query[key] !== undefined).map(key => [key, query[key]]))
...
const assertQueryKeys = (query: Request['query'], allowed: string[]): void => {
    ...
    if (unknown.length) throw new AdminReleaseApiError(400, 'VALIDATION_ERROR', 'Invalid query fields', Object.fromEntries(unknown.map(key => [key, ['Query field is not allowed']])))
}
...
export const createRelease = async (req: Request, res: Response): Promise<Response> => {
    const requestId = requestIdFor(req, res)
    try {
        const payload = parseBody(createReleaseSchema, req.body)
        return sendMutation(res, requestId, await mutateAdminRelease('release.create', null, null, payload, actor(req), requestId), 201)
    } catch (error) { return sendError(req, res, error, 'create release') }
}
...
export const getRelease = async (req: Request, res: Response): Promise<Response> => {
    ...
    try {
        ...
        if (req.query.includeArchived !== undefined && !['true', 'false'].includes(String(req.query.includeArchived))) throw new AdminReleaseApiError(400, 'VALIDATION_ERROR', 'Invalid includeArchived', { includeArchived: ['Use true or false'] })
        if (includeArchived && currentActor.role !== 'admin') throw new AdminReleaseApiError(403, 'FORBIDDEN', 'Only admins may include archived records')
        const release = await getAdminRelease<any>(releaseId, includeArchived, currentActor)
        res.setHeader('Cache-Control', 'no-store')
        res.setHeader('ETag', `"${release.rowVersion}"`)
        return res.json({ data: { release }, meta: successMeta(requestId) })
    } catch (error) { return sendError(req, res, error, 'get release') }
}

export const listReleaseAudit = async (req: Request, res: Response): Promise<Response> => {
    const requestId = requestIdFor(req, res)
    try {
        actor(req)
...
export const updateRelease = async (req: Request, res: Response): Promise<Response> => {
    const requestId = requestIdFor(req, res)
    try {
        const releaseId = parseUuid(req.params.releaseId, 'releaseId')
...
export const releaseAction = (operation: string) => async (req: Request, res: Response): Promise<Response> => {
    ...
    try {
        ...
        return sendMutation(res, requestId, await mutateAdminRelease(operation, releaseId, requireIfMatch(req), payload, actor(req), requestId))
    } catch (error) { return sendError(req, res, error, operation) }
}
...
export const archiveNote = async (req: Request, res: Response): Promise<Response> => {
    const requestId = requestIdFor(req, res)
    try {
        const releaseId = parseUuid(req.params.releaseId, 'releaseId')
        const noteId = parseUuid(req.params.noteId, 'noteId')
        const payload = parseBody(archiveNoteSchema, req.body)
        return sendMutation(res, requestId, await mutateAdminRelease('note.archive', releaseId, requireIfMatch(req, 'note'), { ...payload, noteId }, actor(req), requestId))
    } catch (error) { return sendError(req, res, error, 'archive note') }
}

export const reorderNotes = async (req: Request, res: Response): Promise<Response> => {
    const requestId = requestIdFor(req, res)
    try {
        const releaseId = parseUuid(req.params.releaseId, 'releaseId')
        const payload = parseBody(reorderNotesSchema, req.body)
        return sendMutation(res, requestId, await mutateAdminRelease('note.reorder', releaseId, requireIfMatch(req), payload, actor(req), requestId))
    } catch (error) { return sendError(req, res, error, 'reorder notes') }
}

export const approveSource = async (req: Request, res: Response): Promise<Response> => {
    const requestId = requestIdFor(req, res)
    try {
        const releaseId = parseUuid(req.params.releaseId, 'releaseId')
        const sourceId = parseUuid(req.params.prdId, 'prdId')
        const payload = parseBody(approveSourceSchema, req.body)
        return sendMutation(res, requestId, await mutateAdminRelease('source.approve', releaseId, requireIfMatch(req, 'source'), { ...payload, sourceId }, actor(req), requestId))
    } catch (error) { return sendError(req, res, error, 'approve source') }
}
