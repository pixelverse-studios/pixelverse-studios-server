    ...
    return {
        async invalidate(job) {
            for (const target of job.targets) {
                ...
                if (!response.ok)
                    throw new Error(
                        `Revalidation receiver returned HTTP ${response.status}`
                    )
            }
        }
    }
...
export const dispatchReleaseCacheInvalidations = async (
    ...
    const { data, error } = await domaniDb.rpc(
        'claim_release_cache_invalidation_jobs',
        { p_limit: 10 }
    )
    ...
    for (const job of jobs) {
        try {
            ...
            const { error: completeError } = await domaniDb.rpc(
            ...
            const safeMessage =
                deliveryError instanceof Error
                    ...
                    : 'Delivery failed'
            const { error: failError } = await domaniDb.rpc(
                'fail_release_cache_invalidation_job',
                { p_job_id: job.id, p_error: safeMessage }
            )
            if (failError)
                console.error(
                    'Unable to record release invalidation failure:',
                    { jobId: job.id, message: failError.message }
                )
